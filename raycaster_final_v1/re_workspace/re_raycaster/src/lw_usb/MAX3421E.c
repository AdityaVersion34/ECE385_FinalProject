...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE reg_write[2] = {(reg + 2), val};
	BYTE reg_read[2];
	//will store the cumulative error of two write attempts - should be 0 in the end
	//first write - using XSpi_Transfer

	int cumul_err = XSpi_Transfer(&SpiInstance, reg_write, reg_read, 2);

	//int cumul_err = XSpi_Transfer(&SpiInstance, reg_write, reg_write, 2);
	//second write - the actual data now
	//cumul_err += XSpi_Transfer(&SpiInstance, &val, NULL, 1);

	if (cumul_err != 0){
		xil_printf("ERROR: An error occurred in MAXreg_wr");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE data_clean[nbytes+1];	//init clean data copy array
	data_clean[0] = reg+2;
	for (size_t i=0; i<nbytes; i++){
		data_clean[i+1] = data[i];
	}
	BYTE read_buffer[nbytes+1];

		//write attempt

	int cumul_err = XSpi_Transfer(&SpiInstance, data_clean, read_buffer, nbytes+1);

	if (cumul_err != 0){
			xil_printf("ERROR: An error occurred in MAXbytes_wr");
	}
	for(size_t i=0; i<nbytes; i++){
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE write_val[2] = {reg, 0};
	BYTE read_val[2];

	int cumul_err = XSpi_Transfer(&SpiInstance, write_val, read_val, 2);

	if (cumul_err != 0){
		xil_printf("ERROR: An error occurred in MAXreg_rd");
	}
	XSpi_SetSlaveSelect(&SpiInstance,0);
	return read_val[1];
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance,1);
		...
		//similar to single byte read

	BYTE data_clean[nbytes+1];
	BYTE read_buffer[nbytes+1];
	data_clean[0] = reg;
	for(size_t i=0; i<nbytes; i++){
		read_buffer[i+1] = data[i];
	}

	int cumul_err = XSpi_Transfer(&SpiInstance, data_clean, read_buffer, nbytes+1);

	if (cumul_err != 0){
		xil_printf("ERROR: An error occurred in MAXbytes_rd");
	}
	for(size_t i=0; i<nbytes; i++){
	        data[i] = read_buffer[i+1];
	}
